...
public class SerialServiceDiadem extends Service implements ISerialListener {
    ...
    public SerialServiceDiadem() {
        ...
        BroadcastReceiver EnvioDatosSerialBalanza = new BroadcastReceiver() {
            @Override
            public void onReceive(Context context, Intent intent) {
                String datos = (String) intent.getExtras().get(SerialUtils.EXTRA_MESSAGE);
                try {
                    write(datos.getBytes());
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
        ...
        BroadcastReceiver AccionesSobreServicioBalanza = new BroadcastReceiver() {
            @Override
            public void onReceive(Context context, Intent intent) {
                SerialUtils.ServiceAction serviceAction =
                        (SerialUtils.ServiceAction) intent.getExtras().get(SerialUtils.EXTRA_ACTIONS);
...
//                int caso = (int) intent.getExtras().get(SerialUtils.EXTRA_CASE);
//                String mensaje = (String) intent.getExtras().get(SerialUtils.EXTRA_MESSAGE);
//
//                if(caso == SerialUtils.CASE_DISCONNECT_SERVICE) disconnect();
...
//                else Log.d(Constantes.TAG, "el nombre de dispositivo no coincide, es: " + nombreDispositivo());
        ...
        LocalBroadcastManager.getInstance(this).registerReceiver(EnvioDatosSerialBalanza, new IntentFilter(SerialUtils.FILTER_SEND_TO_SCALE));
        LocalBroadcastManager.getInstance(this).registerReceiver(AccionesSobreServicioBalanza, new IntentFilter(SerialUtils.FILTER_SCALE_ACTIONS));
    }
...
//                .setContentText(socket != null ? "Connected to "+socket.toString() : "Background Service")
...
//        startForeground(SerialUtils.NOTIFY_MANAGER_START_FOREGROUND_SERVICE, notification);
    ...
    public void onSerialConnect() {
        if(connected) {
            ...
            SerialUtils.statusScaleBleService(this, SerialUtils.ServiceStatus.Connected);
        }
    }
    ...
    public void onSerialConnectError(Exception e) {
        if(connected) {
            ...
            SerialUtils.statusScaleBleService(this, SerialUtils.ServiceStatus.Error);
        }
    }
    ...
    public void onSerialRead(byte[] data) {
        if(connected) {
            ...
            SerialUtils.receptionScaleBleService(this, new String(data));
        }
    }
    ...
    public void onSerialIoError(Exception e) {
        if(connected) {
            ...
            SerialUtils.statusScaleBleService(this, SerialUtils.ServiceStatus.ErrorIO);
        }
    }
