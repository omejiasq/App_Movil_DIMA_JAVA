...
public class SerialSocketDiadem extends BluetoothGattCallback {
    ...
    private static final UUID BLUETOOTH_BTM_TX = UUID.fromString("0000b352-0000-1000-8000-00805f9b34fb"); //Used for write data to the modu
    ...
    public void disconnect() {
        ...
        if (gatt != null) {
            ...
            gatt = null;
            connected = false;
            SerialUtils.statusScaleBleService(context, SerialUtils.ServiceStatus.Disconnected);
        }
        try {
    ...
    public void connect(ISerialListener listener) throws IOException {
        ...
        context.registerReceiver(disconnectBroadcastReceiver, new IntentFilter(Constants.Permissions.INTENT_ACTION_DISCONNECT), Context.RECEIVER_NOT_EXPORTED);
        ...
        if (Build.VERSION.SDK_INT < 23) {
            ...
            Log.d(Constants.TAG, "gatt conectado: " + gatt);
        }
        if (gatt == null)
...
//                onSerialConnectError(new IOException(context.getString(R.string.pairing_request)));
    ...
    private void connectCharacteristics1(BluetoothGatt gatt) {
        ...
        Log.d(TAG, "gatt detectado: " + gatt.getServices());
        Log.d(TAG, "dispositivo: " + gatt.getDevice().getName());
...
//                onSerialConnectError(new IOException("request MTU failed"));
//            else {
//                Log.d(Constants.TAG, "now connect characteristic 2 succeed, trying 3?");
//                connectCharacteristics3(gatt);
//            }
//            // continues asynchronously in onMtuChanged
//        } else {
//            Log.d(Constants.TAG, "connectCharacteristic 2 succeed, passing gatt to connectCharacteristics3");
//            connectCharacteristics3(gatt);
//        }
    }
...
//    int descriptorTrying = 0;
//    BluetoothGatt gattTesting = null;
//    BluetoothGattDescriptor gattDescriptor = null;

//    private void startCheck() {
//        handler.post(checkForDescriptor);
//    }
//
//    private void stopCheck() {
//        handler.removeCallbacks(checkForDescriptor);
//    }
//
//    @SuppressLint("MissingPermission")
//    private final Runnable checkForDescriptor = () -> {
//        if(descriptorTrying < 5) {
//            try {
//                boolean isSet = gattTesting.writeDescriptor(gattDescriptor);
//                if(!isSet)
//                    Log.d(Constants.TAG, "gattTesting writeDescriptor is false");
//                else {
//                    Log.d(Constants.TAG, "gattTesting writeDescriptor is true");
//                    stopCheck();
//                }
//            }
//            catch (Exception e) {
...
//            descriptorTrying++;
//            startCheck();
//        } else if (descriptorTrying == 5) {
//            Log.d(Constants.TAG, "5 tries done for descriptor set");
//            stopCheck();
//            onSerialConnectError(new IOException("read characteristic CCCD descriptor not writable"));
//        }
//    };
    ...
    ///endregion INTERFAZ IMPLEMENTADA
    ...
    private class BtmDelegate extends DeviceDelegate {
        ...
        boolean connectCharacteristics(BluetoothGattService gattService) {
            ...
            return true;
//            BluetoothGattCharacteristic readCreditsCharacteristic = gattService.getCharacteristic(BLUETOOTH_BTM_RX);
//            BluetoothGattCharacteristic writeCreditsCharacteristic = gattService.getCharacteristic(BLUETOOTH_BTM_TX);
...
//            }
    ...
    private class BtmDelegate2 extends DeviceDelegate {
        boolean connectCharacteristics(BluetoothGattService gattService) {
            Log.d(TAG, "BtmDelegate 2");
            BluetoothGattCharacteristic rw2 = gattService.getCharacteristic(BLUETOOTH_BTM_RX);
            BluetoothGattCharacteristic rw3 = gattService.getCharacteristic(BLUETOOTH_BTM_TX);
    ...
    private class BtmDelegate3 extends DeviceDelegate {
        ...
        @Override
        boolean connectCharacteristics(BluetoothGattService gattService) {
            Log.d(TAG, "service btm3");
            readCredits = 0;
            writeCredits = 0;
            readCharacteristic = gattService.getCharacteristic(BLUETOOTH_BTM_RX);
            writeCharacteristic = gattService.getCharacteristic(BLUETOOTH_BTM_TX);
            readCreditsCharacteristic = gattService.getCharacteristic(BLUETOOTH_BTM_RX);
            writeCreditsCharacteristic = gattService.getCharacteristic(BLUETOOTH_BTM_TX);
