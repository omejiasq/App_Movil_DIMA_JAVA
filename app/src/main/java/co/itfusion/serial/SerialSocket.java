...
public class SerialSocket extends BluetoothGattCallback {
    ...
    private String dispositivoBalanza;
    private String dispositivoRFID;
    ...
    public SerialSocket(Context context, BluetoothDevice device) {
        ...
        if(device.getName().equals(SerialUtils.NAME_BLE_SCALE_DEVICE)) this.dispositivoBalanza = device.getName();
        if(device.getName().equals(SerialUtils.NAME_BLE_RFID_DEVICE)) this.dispositivoRFID = device.getName();
    ...
    public final String ObtenerNombreBalanza()
    {
        return this.dispositivoBalanza;
    }

    public final String ObtenerNombreRFID()
    {
        return this.dispositivoRFID;
    }
    ...
    public void disconnect() {
        ...
        if (gatt != null) {
            ...
            gatt = null;
            connected = false;
            SerialUtils.EstadoConexionSerial(context, SerialUtils.CASE_BLUETOOTH_DISCONNECTED, device.getName());
        }
        try {
    ...
    public void connect(ISerialListener listener) throws IOException {
        ...
        context.registerReceiver(disconnectBroadcastReceiver, new IntentFilter(SerialUtils.INTENT_ACTION_DISCONNECT));
...
//                onSerialConnectError(new IOException(context.getString(R.string.pairing_request)));
    ...
    private void connectCharacteristics1(BluetoothGatt gatt) {
        ...
        Log.d(TAG, "gatt dispositivo: " + gatt.getDevice().getName());
